...
//小符
        ...
        if (KeyBoard.F.click_flag == 1 && vision_data.mode == 0x24 && robot_ctrl.target_lock == 0x31 &&
            (detect_list[DETECT_AUTO_AIM].status == ONLINE)) {
            gimbal.last_mode = GIMBAL_ACTIVE;
...
static void gimbal_auto_handle() {
//    if(KeyBoard.G.status == KEY_CLICK) a_pitch += 0.1f;
//    if(KeyBoard.V.status == KEY_CLICK) a_pitch -= 0.1f;
//    if(KeyBoard.Z.status == KEY_CLICK) a_yaw += 0.1f;
//    if(KeyBoard.X.status == KEY_CLICK) a_yaw -= 0.1f;
...
//    if(KeyBoard.F.click_flag == 0) {
    gimbal.yaw.absolute_angle_set = atan2f(auto_yaw[0].out, auto_yaw[1].out) * 180.0f / PI + a_yaw;//在此处做合成
//    }
//    else{
    //鼠标输入滤波
//        first_order_filter_cali(&mouse_in_x,rc_ctrl.mouse.x);
...
//                                       +mouse_in_x.out*MOUSE_X_RADIO;
//    }
    gimbal.pitch.absolute_angle_set = auto_pitch.out + a_pitch;
...
int aaa = 400;//测试用变量
//小头1700 846
int open_angle = 800, close_angle = 330, last_angle = 330, last_ch4 = 0;
...
//    if (KeyBoard.G.click_flag == 1 ) {//搓那个滚轮/
