...
//                            float filtercurrent= first_Kalman_Filter(&chassis_filter[i],chassis.motor_chassis[i].motor_measure->given_current);
                            tmp1 += chassis.motor_chassis[i].motor_measure->given_current *
                                    chassis.motor_chassis[i].motor_measure->given_current;
                            tmp2 += chassis.motor_chassis[i].motor_measure->speed_rpm *
                                    chassis.motor_chassis[i].motor_measure->given_current;
                        ...
                        power_nihe = CHASSIS_POWER_R0 * all_current_pingfang + CHASSIS_POWER_K0 * all_rpm_mul_current +
                                     CHASSIS_POWER_P0;
...
//                        break;
                    ...
                    case Referee_ID_robot_interactive_header_data://0x0301
                        memcpy(&Referee.StudentInteractive, ReadFromUsart + DATA,
        ...
        if (*(ReadFromUsart + sizeof(frame_header_struct_t) + Referee_LEN_CMD_ID + Referee.FrameHeader.data_length +
              Referee_LEN_FRAME_TAIL) == 0xA5) {
            //如果一个数据包出现了多帧数据,则再次读取
            Referee_read_data(ReadFromUsart + sizeof(frame_header_struct_t) + Referee_LEN_CMD_ID +
                              Referee.FrameHeader.data_length + Referee_LEN_FRAME_TAIL);
        }
    }
}
...
//    float real_cap = cap_percentage * 216;
    ...
    Figure_Graphic(&ui_cap_percentage.clientData[1], "CAC", UI_MODIFY, UI_LINE,
                   UI_FOUR_LAYER, UI_GREEN, 0, 0, 20, 839, 125, 0, 839, 125);
    //CRC18校验
    memcpy(ClientTxCapBuffer + Referee_LEN_FRAME_HEAD, (uint8_t *) &ui_cap_percentage.CmdID, sizeof(ui_cap_percentage));
    append_CRC16_check_sum(ClientTxCapBuffer, sizeof(ui_cap_percentage));
    usart6_tx_dma_enable(ClientTxCapBuffer, sizeof(ui_cap_percentage));
...
//    }
    if (cap_mode == CAP_MODE_WORK)//按下时电容开，提示颜色为紫红色
    {
        uiColor.cover_color = UI_FUCHSIA;
    } else {
...
void ui_cover_draw_init()//旋转跟弹舱一起
{
    ...
    Figure_Graphic(&ui_cover_spin.clientData[0], "CO2", UI_ADD, UI_CIRCLE, UI_THREE_LAYER, UI_CYAN_BLUE,
                   0, 0, 6, 503, 760, 42, 0, 0);//643
//    Int_Graphic(&ui_cover_spin.clientData[0], "CO2", UI_ADD, UI_INT, UI_THREE_LAYER, UI_CYAN_BLUE,
//                       25, 25, 6, 523, 643, Cap.capFeedback.esr_v);
    Int_Graphic(&ui_cover_spin.clientData[1], "SP2", UI_ADD, UI_INT, UI_THREE_LAYER, UI_CYAN_BLUE,
                24, 0, 3, 543, 643, 0);//760
    //去除帧头部分，其他放入缓存区
...
void dynamic_spin_cover_draw() {
    ...
    ui_cover_spin.dataFrameHeader.data_cmd_id = UI_INTERACT_ID_draw_two_graphic;
    //数据填充
    Int_Graphic(&ui_cover_spin.clientData[0], "CO2", UI_MODIFY, UI_INT, UI_THREE_LAYER, uiColor.cover_color,
                24, 0, 3, 503, 643, Cap.capFeedback.esr_v);
//    Int_Graphic(&ui_cover_spin.clientData[0], "CO2", UI_ADD, UI_INT, UI_THREE_LAYER, UI_CYAN_BLUE,
//                25, 25, 6, 523, 643, Cap.capFeedback.esr_v);
...
void dynamic_pitch_draw() {
    ...
    Figure_Graphic(&ui_pitch_change.clientData, "LIO", UI_MODIFY, UI_ARC, UI_ZERO_LAYER, UI_ORANGE,
                   (unsigned long) gimbal.pitch.relative_angle_get + 32 - 1,
                   (unsigned long) gimbal.pitch.relative_angle_get + 32 + 1, 45, 960, 539, 0, 360, 360);
