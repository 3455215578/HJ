...
//    CAN_cmd_motor(CAN_1,0x700,0,0,0,0);
    //主任务循环
    while (1) {
        ...
        if (chassis.mode != CHASSIS_RELAX) {
            ...
            //  chassis_vector_powerControl();ds
            //底盘解算
            chassis_wheel_cal();
            //功率限制asdsdasadasdasdassaasdssadasdasdas
            if (detect_list[DETECT_CAP].status == ONLINE && cap_mode == CAP_MODE_WORK) {
                buffer_limit = Cap.capFeedback.esr_v > 400 ? 50 - (((fp32) Cap.capFeedback.esr_v) / 2800) * 40 : 50;
...
//                    chassis_power_limit(50, 0);
                k = Cap.capFeedback.esr_v > 1500 ? 0.15f : (fp32) (Cap.capFeedback.esr_v) / 2800 * 0.15f;
                chassis_power_limit(buffer_limit, k);
            } else if (detect_list[DETECT_CAP].status == OFFLINE || cap_mode == CAP_MODE_SILENT) {
                chassis_power_limit(50, 0);
            }
            //驱电机闭环
...
static void chassis_init(chassis_t *chassis_ptr) {
    ...
    ramp_init(&chassis_3508_ramp[LF], 0.00002f, M3508_MAX_RPM, -M3508_MAX_RPM);
    ramp_init(&chassis_3508_ramp[RF], 0.00002f, M3508_MAX_RPM, -M3508_MAX_RPM);
    ramp_init(&chassis_3508_ramp[RB], 0.00002f, M3508_MAX_RPM, -M3508_MAX_RPM);
    ramp_init(&chassis_3508_ramp[LB], 0.00002f, M3508_MAX_RPM, -M3508_MAX_RPM);
...
fp32 tf_rad = 0.f;
fp32 self_weight = 10.f;
fp32 chassis_pitch = 0.f;
fp32 torque_to_current = 60.0f;
fp32 mg_compensation_torque = 0.f;
...
static void chassis_wheel_loop_cal() {
    ...
    tf_rad = motor_ecd_to_rad_change(gimbal.pitch.motor_measure->ecd, 2281);
    chassis_pitch = INS_angle[2] - tf_rad;
    mg_compensation_torque = self_weight * 9.8f * sinf(chassis_pitch);
    if (fabsf(mg_compensation_torque) > mg_compensation_torque_limit) {
        if (mg_compensation_torque > 0)
            mg_compensation_torque = mg_compensation_torque_limit;
        else
            mg_compensation_torque = -mg_compensation_torque_limit;
    }
    mg_compensation_current = mg_compensation_torque * torque_to_current;
    if (fabsf(mg_compensation_current) > mg_compensation_current_limit) {
        if (mg_compensation_current > 0)
            mg_compensation_current = mg_compensation_current_limit;
        else
            mg_compensation_current = -mg_compensation_current_limit;
    }
    ...
    chassis.motor_chassis[RF].give_current = (int16_t) pid_calc(&chassis.motor_chassis[RF].speed_p,
                                                                ...
                                                                chassis.motor_chassis[RF].rpm_set);// + (int16_t)mg_compensation_current;
    ...
    chassis.motor_chassis[LF].give_current = (int16_t) pid_calc(&chassis.motor_chassis[LF].speed_p,
                                                                ...
                                                                chassis.motor_chassis[LF].rpm_set);// + (int16_t)mg_compensation_current;
    ...
    chassis.motor_chassis[RB].give_current = (int16_t) pid_calc(&chassis.motor_chassis[RB].speed_p,
                                                                ...
                                                                chassis.motor_chassis[RB].rpm_set);// + (int16_t)mg_compensation_current;
    ...
    chassis.motor_chassis[LB].give_current = (int16_t) pid_calc(&chassis.motor_chassis[LB].speed_p,
                                                                ...
                                                                chassis.motor_chassis[LB].rpm_set);//+ (int16_t)mg_compensation_current;
...
void chassis_power_limit(fp32 buffer_limit, fp32 k) {
    if (detect_list[DETECT_CAP].status == ONLINE) {
        ...
        chassis.chassis_power_limit.power_set = Referee.GameRobotStat.chassis_power_limit; //+ (Referee.PowerHeatData.chassis_power_buffer-buffer_limit);
    }
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        b += chassis.motor_chassis[i].rpm_set *
             (2 * CHASSIS_POWER_R0 * chassis.chassis_power_limit.K[i] * chassis.chassis_power_limit.M[i] +
              CHASSIS_POWER_K0 * chassis.chassis_power_limit.K[i] * chassis.motor_chassis[i].motor_measure->speed_rpm);
        c += CHASSIS_POWER_R0 * pow(chassis.chassis_power_limit.M[i], 2) +
             CHASSIS_POWER_K0 * chassis.chassis_power_limit.M[i] * chassis.motor_chassis[i].motor_measure->speed_rpm;
    ...
    if (Referee.PowerHeatData.chassis_power_buffer < buffer_limit) {
        ...
        c += (buffer_limit - chassis.chassis_power_limit.power_buff) * 1.0f;
    }
    //dangerous=true;
...
//    fp32 vy_temp=chassis.vy;
    ...
    VAL_LIMIT(chassis.vw, -MAX_CHASSIS_VW_SPEED, MAX_CHASSIS_VW_SPEED);
    VAL_LIMIT(chassis.vx, -MAX_CHASSIS_VX_SPEED * (1 - abs(chassis.vw / 4.4)),
              MAX_CHASSIS_VX_SPEED * (1 - abs(chassis.vw / 4.4)));
    VAL_LIMIT(chassis.vy, -MAX_CHASSIS_VY_SPEED * (1 - abs(chassis.vw / 4.4)),
              MAX_CHASSIS_VY_SPEED * (1 - abs(chassis.vw / 4.4)));
}
...
static void chassis_pc_ctrl() {
    ...
    VAL_LIMIT(chassis.vy_pc, -MAX_CHASSIS_VY_SPEED, MAX_CHASSIS_VY_SPEED);
...
static float chassis_speed_change() {
    ...
    if (detect_list[DETECT_CAP].status == ONLINE) //开启电容 增加加速度
    {
