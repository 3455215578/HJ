...
void cap_init(uint8_t cap_mode_)
{
    ...
    if(cap_mode_ == CAP_MODE_SILENT){
        cap_mode = CAP_MODE_SILENT;
    }else if(cap_mode_ == CAP_MODE_WORK){
        cap_mode = CAP_MODE_WORK;
    }
}

void cap_control_ExceedOn(uint8_t pb_set, uint8_t cap_mode_set)
{
    uint8_t can_send_data[8];
    ...
    if(cap_mode_set == CAP_MODE_SILENT){
        cap_mode = CAP_MODE_SILENT;
    }else if(cap_mode_set == CAP_MODE_WORK){
        cap_mode = CAP_MODE_WORK;
    }
}

void cap_control_ExceedOff(uint8_t pb_set, uint8_t cap_mode_set)
{
    uint8_t can_send_data[8];
    tx_message.StdId = CAP_CONTROL_ID;
    tx_message.IDE = CAN_ID_STD;
