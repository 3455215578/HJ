//
// Created by liang on 2025-02-16.
//

#include "Feedforward_PID.h"

void FD_PID_Init(F_PID_t *PID, const fp32 parameters[PARAMS]) {
...
void VSP_PID_Init(VSP_PID_t *VSP, const fp32 parameters[PARAMS], fp32 k0, fp32 k1, fp32 k2, fp32 min_kp) {
    FD_PID_Init(&VSP->controller, parameters);
    VSP->k[0] = k0;
    VSP->k[1] = k1;
    VSP->k[2] = k2;
...
fp32 VSP_PID_Calc(VSP_PID_t *VSP, fp32 feedback, fp32 reference) {
    ...
    VSP->controller.particle_i +=
            VSP->controller.parameters[KI] * (VSP->controller.error[PREV] + VSP->controller.error[CURR]);
    VSP->controller.particle_d =
            VSP->controller.parameters[KD] * (VSP->controller.error[CURR] - VSP->controller.error[PREV]);
