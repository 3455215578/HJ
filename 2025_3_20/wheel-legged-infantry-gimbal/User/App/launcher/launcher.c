...
void launcher_mode_set(){
    ...
    int shoot_enable = (switch_is_down(rc_ctrl.rc.s[RC_s_L]) || (KeyBoard.Mouse_l.status == KEY_CLICK)) || (robot_ctrl.fire_command != 0);
...
bool thisReverseFlag =false;
bool lastReverseFlag=false;
int next_goal=0;
int next_flag=false;   //记录单发堵转的目标ecd，以及是否进行单发堵转处理
...
//        }
    ...
    /** 发射机构pid初始化 **/
    void launcher_pid_init(void)
    {
        /** 摩擦轮速度环pid **/
        pid_init(&launcher.fire_l.speed_p,
                 SHOOT_FIRE_L_PID_MAX_OUT,
                 SHOOT_FIRE_L_PID_MAX_IOUT,
                 SHOOT_FIRE_L_PID_KP,
                 SHOOT_FIRE_L_PID_KI,
                 SHOOT_FIRE_L_PID_KD);

        pid_init(&launcher.fire_r.speed_p,
                 SHOOT_FIRE_R_PID_MAX_OUT,
                 SHOOT_FIRE_R_PID_MAX_IOUT,
                 SHOOT_FIRE_R_PID_KP,
                 SHOOT_FIRE_R_PID_KI,
                 SHOOT_FIRE_R_PID_KD);
        ...
        pid_init(&launcher.trigger.angle_p,
                 SHOOT_TRI_ANGLE_PID_MAX_OUT,
                 SHOOT_TRI_ANGLE_PID_MAX_IOUT,
                 SHOOT_TRI_ANGLE_PID_KP,
                 SHOOT_TRI_ANGLE_PID_KI,
                 SHOOT_TRI_ANGLE_PID_KD);

        /** 拨盘速度环pid **/
        pid_init(&launcher.trigger.speed_p,
                 SHOOT_TRI_SPEED_PID_MAX_OUT,
                 SHOOT_TRI_SPEED_PID_MAX_IOUT,
                 SHOOT_TRI_SPEED_PID_KP,
                 SHOOT_TRI_SPEED_PID_KI,
                 SHOOT_TRI_SPEED_PID_KD);
    }
    ...
    void launcher_init() {
        ...
        /** 发射机构初始化 **/
        // 拨盘
        launcher.trigger_cmd = SHOOT_CLOSE;//初始时拨盘默认关闭

        // 摩擦轮
        launcher.fire_mode = launcher.fire_last_mode = Fire_OFF;//初始时摩擦轮默认关闭
