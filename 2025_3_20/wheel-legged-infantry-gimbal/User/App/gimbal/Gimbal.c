...
static void gimbal_motor_cmd_send();
...
/** 自瞄云台pid **/
    //yaw
    pid_init(&gimbal.yaw.angle_pid_auto,
    ...
    pid_init(&gimbal.yaw.speed_pid_auto,
    ...
    //pitch
    pid_init(&gimbal.pitch.angle_pid_auto,
    ...
    pid_init(&gimbal.pitch.speed_pid_auto,
...
//        first_order_filter_cali(&mouse_in_x,Referee.keyboard.mouse_x);
    ...
    //云台期望设置
    gimbal.yaw.absolute_angle_set -=  (float)rc_ctrl.rc.ch[YAW_CHANNEL] * RC_TO_YAW * GIMBAL_RC_MOVE_RATIO_YAW
...
//        first_order_filter_cali(&mouse_in_y,Referee.keyboard.mouse_y);
    ...
    //云台期望设置
    gimbal.pitch.absolute_angle_set += (float)rc_ctrl.rc.ch[PITCH_CHANNEL] * RC_TO_PITCH * GIMBAL_RC_MOVE_RATIO_PIT
...
static void gimbal_ctrl_loop_cal(){
    ...
    /** yaw **/
    // 角度环
    gimbal.yaw.gyro_set = pid_loop_calc(&gimbal.yaw.angle_pid,
    ...
    // 速度环
    gimbal.yaw.give_current = pid_calc(&gimbal.yaw.speed_pid,
                                       filter_yaw_gyro_in.out,
                                       gimbal.yaw.gyro_set);

    /** pitch **/
    // 角度环
    gimbal.pitch.gyro_set = pid_calc(&gimbal.pitch.angle_pid,
    ...
    // 速度环
    gimbal.pitch.give_current = pid_calc(&gimbal.pitch.speed_pid,
...
//    gimbal.yaw.angle_p.p=yaw_angle_p;
    gimbal.yaw.gyro_set= pid_loop_calc(&gimbal.yaw.angle_pid_auto,
                                       gimbal.yaw.absolute_angle_get,
                                       gimbal.yaw.absolute_angle_set,
                                       180,
                                       -180);
    ...
    gimbal.yaw.give_current= pid_calc(&gimbal.yaw.speed_pid_auto,
                                      filter_yaw_gyro_in.out,
                                      gimbal.yaw.gyro_set);


    gimbal.pitch.gyro_set= pid_calc(&gimbal.pitch.angle_pid_auto,
                                    gimbal.pitch.absolute_angle_get,
                                    gimbal.pitch.absolute_angle_set);

    first_order_filter_cali(&filter_pitch_gyro_in,gimbal.absolute_gyro_pitch);

    gimbal.pitch.give_current=pid_calc(&gimbal.pitch.speed_pid_auto,
                                       filter_pitch_gyro_in.out,
                                       gimbal.pitch.gyro_set);//加负号为了电机反转
