...
void vmc_phi_update(Leg *leg_L, Leg *leg_R) {
    ...
    leg_L->vmc.forward_kinematics.fk_phi.phi4 = PI/2 + LF_joint_pos - 2.2055f;
    leg_R->vmc.forward_kinematics.fk_phi.phi4 = PI/2 - RF_joint_pos - 2.2055f;

    leg_L->vmc.forward_kinematics.fk_phi.phi1 = PI/2 + LB_joint_pos + 2.2057f;
    leg_R->vmc.forward_kinematics.fk_phi.phi1 = PI/2 - RB_joint_pos + 0.2568f;
