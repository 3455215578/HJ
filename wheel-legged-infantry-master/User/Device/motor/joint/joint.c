#include <cmsis_os.h>
#include "joint.h"

Dm8009 joint[4];

void joint_disable() {

    dm8009_init(&joint[LF], JOINT_LF_SEND);
    dm8009_init(&joint[LB], JOINT_LB_SEND);
    dm8009_init(&joint[RF], JOINT_RF_SEND);
    dm8009_init(&joint[RB], JOINT_RB_SEND);

    set_dm8009_disable(CAN_2, JOINT_LF_SEND);
    set_dm8009_disable(CAN_2, JOINT_LB_SEND);
    set_dm8009_disable(CAN_2, JOINT_RF_SEND);
    set_dm8009_disable(CAN_2, JOINT_RB_SEND);

}

void joint_enable() {

    dm8009_init(&joint[LF], JOINT_LF_SEND);
    dm8009_init(&joint[LB], JOINT_LB_SEND);
    dm8009_init(&joint[RF], JOINT_RF_SEND);
    dm8009_init(&joint[RB], JOINT_RB_SEND);

    set_dm8009_enable(CAN_2, JOINT_LF_SEND);
    set_dm8009_enable(CAN_2, JOINT_LB_SEND);
    set_dm8009_enable(CAN_2, JOINT_RF_SEND);
    set_dm8009_enable(CAN_2, JOINT_RB_SEND);

}

void set_joint_torque(float joint_LF_torque, float joint_LB_torque, float joint_RF_torque, float joint_RB_torque) {

  set_dm8009_MIT(CAN_2,JOINT_LF_SEND,0,0,0,0,joint_LF_torque);
  osDelay(1);
  set_dm8009_MIT(CAN_2,JOINT_LB_SEND,0,0,0,0,joint_LB_torque);
  osDelay(1);
  set_dm8009_MIT(CAN_2,JOINT_RF_SEND,0,0,0,0,joint_RF_torque);
  osDelay(1);
  set_dm8009_MIT(CAN_2,JOINT_RB_SEND,0,0,0,0,joint_RB_torque);
}

Dm8009* get_joint_motors(){
  return joint;
}
